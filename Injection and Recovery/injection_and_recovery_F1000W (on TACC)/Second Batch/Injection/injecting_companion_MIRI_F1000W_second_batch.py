...
# Paths
base_folder = Path.home()/"injection_and_recovery_F1000W_secondbatch"
...
# This part below is based on the scatter plot of recovered binaries in the first batch
# Choose 3 points for the upper curve
sep_upper = np.array([1.0, 5.0])
contr_upper = np.array([0.0, 0.0])

# Choose 3 points for the lower curve
sep_lower = np.array([1.0, 5.0])
contr_lower = np.array([2.5, 4.0])

coefs_upper = np.polyfit(sep_upper, contr_upper, 1)
a_upper, b_upper = coefs_upper

coefs_lower = np.polyfit(sep_lower, contr_lower, 1)
a_lower, b_lower = coefs_lower

def line(sep, a, b):
    return a * sep + b 

sep_lower_plot = np.linspace(0, 5, 100)
contr_lower_plot = line(sep_lower_plot, a_lower, b_lower)

sep_upper_plot = np.linspace(0, 5, 100)
contr_upper_plot = line(sep_upper_plot, a_upper, b_upper)
...
for i in range(trials):
    ...
    approved_lower_contr_lim = line(sep, a_lower, b_lower)
    approved_higher_contr_lim = line(sep, a_upper, b_upper) 
    if approved_lower_contr_lim >= min_contrast:
        approved_lower_contr_lim = min_contrast
    if approved_higher_contr_lim <= 0.0:
        approved_higher_contr_lim = 0.0
        
    while not (approved_higher_contr_lim <= contrast <= approved_lower_contr_lim):
        contrast = np.random.rand() * min_contrast
